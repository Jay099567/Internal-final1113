...
# ================================================================================
# MASS SCALE AUTONOMOUS SYSTEM API ENDPOINTS
# ================================================================================

# Master Automation Orchestrator Endpoints
@api_router.post("/automation/start")
async def start_autonomous_system(background_tasks: BackgroundTasks):
    """Start the autonomous job hunting system for all candidates"""
    try:
        # Start the autonomous system in background
        background_tasks.add_task(master_orchestrator.start_autonomous_system)
        
        return {
            "success": True,
            "message": "Autonomous system starting up",
            "status": "initializing"
        }
        
    except Exception as e:
        logger.error(f"Error starting autonomous system: {str(e)}")
...
async def stop_autonomous_system():
    """Stop the autonomous job hunting system"""
    try:
        await master_orchestrator.stop_autonomous_system()
        ...
        logger.error(f"Error stopping autonomous system: {str(e)}")
...
async def get_automation_status():
    ...
    try:
        status = await master_orchestrator.get_system_status()
...
async def get_automation_stats():
    """Get detailed statistics from the autonomous system"""
    try:
        stats = master_orchestrator.stats
        ...
        return {
            ...
            "stats": {
                ...
                "resumes_tailored": stats.resumes_tailored,
                "cover_letters_generated": stats.cover_letters_generated,
...
@api_router.post("/linkedin/start-outreach")
async def start_linkedin_outreach(candidate_id: str, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        background_tasks.add_task(linkedin_automation.start_outreach_campaign, candidate_id)
        
        return {
            "success": True,
            "message": f"LinkedIn outreach started for candidate {candidate_id}",
            "candidate_id": candidate_id
        }
        
    except Exception as e:
        logger.error(f"Error starting LinkedIn outreach: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Failed to start LinkedIn outreach: {str(e)}")

@api_router.get("/linkedin/outreach-status/{candidate_id}")
async def get_outreach_status(candidate_id: str):
    """Get LinkedIn outreach status for a candidate"""
    try:
        status = await linkedin_automation.get_outreach_status(candidate_id)
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get outreach status: {str(e)}")

@api_router.get("/linkedin/campaigns")
async def get_outreach_campaigns():
    """Get all active LinkedIn outreach campaigns"""
    try:
        campaigns = await linkedin_automation.get_active_campaigns()
        ...
        logger.error(f"Error getting outreach campaigns: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Failed to get outreach campaigns: {str(e)}")

# Feedback Learning Loop Endpoints
@api_router.post("/feedback/analyze-performance")
async def analyze_performance():
    """Analyze system performance and generate optimization recommendations"""
    try:
        performance_data = await feedback_analyzer.collect_performance_data()
        recommendations = await feedback_analyzer.generate_optimization_recommendations(performance_data)
...
@api_router.post("/feedback/apply-optimizations")
async def apply_optimizations():
    """Apply recommended optimizations to the system"""
    try:
        result = await feedback_analyzer.apply_optimizations()
        
        return {
            "success": True,
            "optimizations_applied": result
        }
...
@api_router.get("/feedback/success-patterns")
async def get_success_patterns():
    """Get identified success patterns from application data"""
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get success patterns: {str(e)}")

# Mass Scale Analytics Endpoints
@api_router.get("/analytics/mass-scale-dashboard")
async def get_mass_scale_dashboard():
    """Get comprehensive dashboard data for 100+ candidates"""
    try:
        ...
        applications_today = await db.applications.count_documents({
            "applied_at": {"$gte": datetime.now().replace(hour=0, minute=0, second=0, microsecond=0)}
        })
        
        # Get matching statistics
        total_matches = await db.job_matches.count_documents({})
        high_priority_matches = await db.job_matches.count_documents({"priority_score": {"$gte": 0.8}})
        
        # Get outreach statistics
        total_outreach = await db.outreach_messages.count_documents({})
        outreach_responses = await db.outreach_messages.count_documents({"status": "replied"})
        
        # Calculate success rates
        application_success_rate = 0
        if total_applications > 0:
            successful_applications = await db.applications.count_documents({"status": {"$in": ["interview", "offer"]}})
            application_success_rate = (successful_applications / total_applications) * 100
        
        outreach_response_rate = 0
        if total_outreach > 0:
            outreach_response_rate = (outreach_responses / total_outreach) * 100
        
        # Get automation system status
        automation_status = await master_orchestrator.get_system_status()
        ...
        return {
            ...
            "dashboard": {
                "candidates": {
                    ...
                    "inactive": total_candidates - active_candidates
                },
                "jobs": {
                    "total": total_jobs,
                    "scraped_today": jobs_today,
                    "scraping_active": automation_status.get("scraping_active", False)
                },
                "applications": {
                    "total": total_applications,
                    "submitted_today": applications_today,
                    "success_rate": round(application_success_rate, 2)
                },
                "matching": {
                    "total_matches": total_matches,
                    "high_priority": high_priority_matches,
                    "match_rate": round((total_matches / max(total_jobs, 1)) * 100, 2)
                },
                "outreach": {
                    "total_sent": total_outreach,
                    "responses": outreach_responses,
                    "response_rate": round(outreach_response_rate, 2)
                },
                "automation": {
                    "status": automation_status.get("status", "stopped"),
                    "uptime": automation_status.get("uptime", "0h"),
                    "last_cycle": automation_status.get("last_cycle", "Never"),
                    "processed_today": automation_status.get("processed_today", 0)
                }
            }
        }
        
    except Exception as e:
        logger.error(f"Error getting mass scale dashboard: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Failed to get dashboard data: {str(e)}")

@api_router.get("/analytics/candidate-performance/{candidate_id}")
async def get_candidate_performance(candidate_id: str):
    """Get detailed performance analytics for a specific candidate"""
    try:
        # Verify candidate exists
        candidate = await db.candidates.find_one({"id": candidate_id})
        if not candidate:
        ...
        applications = await db.applications.find({"candidate_id": candidate_id}).to_list(1000)
        matches = await db.job_matches.find({"candidate_id": candidate_id}).to_list(1000)
        outreach_messages = await db.outreach_messages.find({"candidate_id": candidate_id}).to_list(1000)
        
        # Calculate performance metrics
        total_applications = len(applications)
        successful_applications = len([app for app in applications if app.get("status") in ["interview", "offer"]])
        
        total_matches = len(matches)
        high_quality_matches = len([match for match in matches if match.get("priority_score", 0) >= 0.8])
        
        total_outreach = len(outreach_messages)
        outreach_responses = len([msg for msg in outreach_messages if msg.get("status") == "replied"])
        ...
        return {
            ...
            "performance": {
                ...
                "matching": {
                    ...
                    "quality_rate": round((high_quality_matches / max(total_matches, 1)) * 100, 2)
                },
                "outreach": {
                    "total_sent": total_outreach,
                    "responses": outreach_responses,
                    "response_rate": round((outreach_responses / max(total_outreach, 1)) * 100, 2)
                }
            }
        }
        
    except Exception as e:
        logger.error(f"Error getting candidate performance: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Failed to get candidate performance: {str(e)}")

# ================================================================================
# END MASS SCALE AUTONOMOUS SYSTEM ENDPOINTS
# ================================================================================
